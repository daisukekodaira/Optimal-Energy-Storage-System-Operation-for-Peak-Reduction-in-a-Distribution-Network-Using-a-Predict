function CI_desc_r2(load,f_name_kernel,f_name_histo,percent)
global_var_declare;
...
for i=1:24
        if percent==0
            ...
            size_initial = size(hourly_class(i).data,1);
            front_size(i) = size_initial*(percent/100);
            back_size(i) = size_initial*((100-percent)/100);
            pred_max(i) = hourly_sort_class(i).data(back_size(i)); %Value of MW(upper line)
            pred_min(i) = hourly_sort_class(i).data(front_size(i)); %Value of MW(lower line)
            m(i) = mean(hourly_class(i).data);
            s(i) = std(hourly_class(i).data);
...
for i=1:24
    ...
    hold off
    title(strcat('Predicted Kernel between(', num2str(i-1), ':00and ', num2str(i), ':00) [',num2str(100-2*percent),'%]' ))
    legend('Kenel distribution',strcat(num2str(100-2*percent),'% CI(Upper)'),strcat(num2str(100-2*percent),'% CI(Lower)'),'mean')
...
for i=1:24
    ...
    title( strcat('Histogram between(', num2str(i-1), ':00and ', num2str(i), ':00) [',num2str(100-2*percent),'%]' ))
    xlabel('Load [MW]'); % x-axis label
