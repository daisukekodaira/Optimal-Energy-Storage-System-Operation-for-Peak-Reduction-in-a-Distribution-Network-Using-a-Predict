%% file open
function [pred_min, pred_max] = confd_range_af(flow_on_feeder)
global_var_declare;
...
for position = 1:size(flow_on_feeder,2) %%flow_on_feeder <=flow_on_feeder
    ...
    for j=1:g_s_period
        SUM_CI=1;
        for k=0.01:0.01:4
            if SUM_CI<=size(hourly_histgram(position,j).data,1)*0.95
                ci=(abs(hourly_histgram(position, j).data-m(position,j))<=s(position,j)*k);
                SUM_CI=sum(ci);
            else
                CI(position,j).data=k;
                save_CI(position,j).data=SUM_CI;
                break;
            end
        end
    end
    ...
    for i = 1: 24 
    pred_max(position,i) = m(position,i) + CI(position,i).data * s(position,i);         % 95% normal confidence interval
    pred_min(position,i) = m(position,i) - CI(position,i).data * s(position,i);
    end    
end
end 