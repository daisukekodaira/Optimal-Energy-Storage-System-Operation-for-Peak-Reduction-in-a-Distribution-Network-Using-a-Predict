...
% percent=20:2.5:2.5;
%% 馆汗侩 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 历厘 
for percent=2.5:2.5:20;
CI_name_raw=strcat('kernel distribution_raw_r2_',num2str(100-2*percent),'%');
CI_name_adj=strcat('kernel distribution_adj_r2_',num2str(100-2*percent),'%');
histo_name_raw=strcat('histogram_raw_r2_',num2str(100-2*percent),'%');
histo_name_adj=strcat('histogram_adj_r2_',num2str(100-2*percent),'%');
...
[g_pred_min, g_pred_max] = Confd_range_af_2(raw_train_load,percent);
run_pso;
...
for position = 1:g_num_ESS+3
    if pos_graph(position) == 1
        ...
        [orig_mn_af, orig_mx_af] = Confd_range_af_2(raw_train_load,percent);
        ...
        [adj_mn, adj_mx] = Confd_range_af_2(adj_train_load,percent);   % mx has 5*24, mn has 5*24    
        y(end+1).data = repelem(adj_mx(position,:),g_coef); % 2min: 1*24, 30 = 720   15min:1*24,4 = 96
...
CI_desc_r2(raw_train_load(1).data, CI_name_raw, histo_name_raw, percent)
CI_desc_r2(adj_train_load(1).data, CI_name_adj, histo_name_adj, percent)
...
fullscreen(q);
fullscreen(w);
Fig_1_name=strcat('figure(1)_',num2str(100-2*percent),'%'); %%%% 馆汗侩
Fig_2_name=strcat('figure(2)_',num2str(100-2*percent),'%');
mkdir('figure');
fpath = pwd;
saveas(figure(1), fullfile(fpath, '/', 'figure', Fig_1_name), 'tif');
saveas(figure(2), fullfile(fpath, '/', 'figure', Fig_2_name), 'tif');
close(figure(1));
close(figure(2));
end
