%---------------------------------------------------------------------------------
% This function is called from "simul_pso_1.m"
% Make the Confidence Interval for PSO objective function
%---------------------------------------------------------------------------------

function [pred_min, pred_max] = Confd_range(flow_on_feeder)
...
for position = 1:size(flow_on_feeder,2)
    ...
    for i = 1:g_s_period  % loop is not necessary maybe
        ...
        m(position,i) = mean(h);
        s(position,i) = std(h); % Just take standard deviation from histogram. If the histogram is not Normal distribution, std is meaning less. 
        pred_min(position,i) = m(position,i) - g_sigma*s(position,i);        
        pred_max(position,i) = m(position,i) + g_sigma*s(position,i);
    end
end 
