...
%     plot(load)
    global_var_declare;
    if size(re_in,2) == 24
        ess_op(:,1) = transpose(repelem(re_in(1,:), g_coef)); % hourly data is expaned to every 2 min data seriese by multiplying 30
        ess_op(:,2) = transpose(repelem(re_in(2,:), g_coef));
    else
      ess_op(:,1) = re_in(:,1);
      ess_op(:,2) = re_in(:,2);
    end
