...
epoch   = 500;  % max iterations

% PSO performance related parameters
ismaximize = 0;
errgrad = 1e-99;   % lowest error gradient tolerance
errgraditer=10^6; % max # of epochs without error change >= errgrad
...
for n = 1:g_num_ESS
    A(n).data = [-1*g_ESS_capacity(n).*ones(g_s_period,1) g_ESS_capacity(n).*ones(g_s_period,1)];
end
