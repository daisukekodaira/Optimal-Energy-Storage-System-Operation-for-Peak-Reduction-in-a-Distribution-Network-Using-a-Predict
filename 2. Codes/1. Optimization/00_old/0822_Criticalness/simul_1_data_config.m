...
f1 = 'load2016.xlsx';
% f2 = 'amb_temp.xlsx';
g_load_train = xlsread(f1,1);
g_load_test = xlsread(f1,2);
% g_amb_temp = xlsread(f2,1);

%% Paramters & Flags
g_days = size(g_load_train,2);   % how many days compose the one group(class)
g_steps = size(g_load_train,1);  % how many time steps in a day : e.x) 2min data =720
...
g_line_capacity = 5;   % Around 10 is appropriate for real data. Interval must be 0.5 such as 9.5, 10.0 or 10.5. I will make it flexible later.
