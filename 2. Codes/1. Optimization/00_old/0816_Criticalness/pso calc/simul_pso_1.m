...
for i = 1:g_num_ESS
    ESS_opt(:,i) = transpose(repelem(out_reshape(i,:),30));   
end
...
%     plot(raw_train_load(position).data)
    if pos_graph(position) == 1 
        ...
        y(end+1).data = repelem(normalized_orig_crit,30);
        ...
        % 8. Adjusted Predicted load
%         ESS_opt = zeros(size(ESS_opt,1),size(ESS_opt,2));
%         plot(raw_train_load(position).data)
        adj_load =  load_calc(raw_train_load(1).data, ESS_opt); 
%         plot(adj_load(position).data)
        if size(g_load_train,2) == 1
