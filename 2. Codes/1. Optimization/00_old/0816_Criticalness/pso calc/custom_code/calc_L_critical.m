...
function [L_critical, switching_cost] = calc_L_critical(in)   
    ...
    if g_flag_crit == 2
        ess_op = repelem(in, 30); % hourly data is expaned to every 2 min data seriese by multiplying 30          
        for day = 1:g_days
            ...
            if day == 1
                for hour = 1: g_s_period % 1:24
                    hourly_histgram(hour).data = g_line_temp(day,(hour-1)*30+1:hour*30); % hourly histgram (for 24 hours) is composed here. Because of 2min data, 30 is multiplied.
                end
            else
                for hour = 1: g_s_period % 1:24
                    hourly_histgram(hour).data = [hourly_histgram(hour).data g_line_temp(day,(hour-1)*30+1:hour*30)];
                end
            end
        end
    ...
    for m = 1:g_num_ESS % the number of ESS
        ...
        if (last_SOC < g_initial_SOC(m) - 0.2) || (g_initial_SOC(m) + 0.2 < last_SOC)
            ...
            % -------------------------------------------------------------------------------------------------------------
            L_critical = ones(size(flow_on_feeder,2), 24).*(10^7 + abs(g_initial_SOC(m) - last_SOC));
            return;
        end
    ...
    % 4. Calculate the number of switching ( charge <-> discharge)
    switching_cost = 0;
    %         for i = 2:g_s_period
    %             if in(i) < 0
    %                 if in(i-1) > 0
    %                     switching_cost = switching_cost + 0.1;
    %                 end
    %             else 
    %                 if in(i-1) < 0
    %                     switching_cost = switching_cost + 0.1;
