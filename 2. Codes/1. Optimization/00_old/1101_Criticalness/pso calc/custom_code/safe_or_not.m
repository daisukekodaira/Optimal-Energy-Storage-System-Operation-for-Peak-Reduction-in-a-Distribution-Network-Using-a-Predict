function [non_ope, L_critical, pdf_parms] = safe_or_not(load)
    global_var_declare;
    
    
    re_in = zeros(g_num_ESS, g_s_period); % ESS operation (all zero)
    [flow_on_feeder] = load_calc(load, re_in);  % get adjusted power flow by ESS operation (ESS operation is zero here)
