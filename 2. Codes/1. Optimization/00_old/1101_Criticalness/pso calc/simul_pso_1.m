...
% pdf_parms : (variance É–, mean É ) of PDFs derived from original load
[non_ope, L_critical_orig, pdf_parms_train] = safe_or_not(g_load_train);

% Get variance and mean of test load data
[non_ope, L_critical_orig, pdf_parms_test] = safe_or_not(g_load_test);
...
for position = 1:g_num_ESS+3
    if pos_graph(position) == 1 
        ...
        % 6. Predicted Criticalness (training + ess_opt)
        L_critical_best = round(g_L_critical_best(position,:),1);
        normalized_adj_crit = 100*L_critical_best/max(L_critical_orig(1,:)); % 1*24
        if isnan(normalized_adj_crit)
            normalized_adj_crit = zeros(size(normalized_adj_crit,1),size(normalized_adj_crit,2));
        end
        y(end+1).data = repelem(normalized_adj_crit,g_coef); % 2min: 1*24, 30 = 720   15min:1*24,4 = 96
        ...
        y(end+1).data = repelem(normalized_orig_crit,g_coef); % 2min: 1*24, 30 = 720   15min:1*24,4 = 96
