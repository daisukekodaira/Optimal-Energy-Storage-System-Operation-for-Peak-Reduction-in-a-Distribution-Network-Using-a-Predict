...
%     switching_cost = zeros(n_,1);
    ...
    for i = 1:n_
        ...
        if max(max(transpose(Crit(i).data))) > 10^5
            % Evaluate SOC constraints
%             out(i) = max(max(transpose(Crit(i).data))) + 100*sum(in(i,:).^2);   % coeffient 100 should be considered to be proper
                out(i) = max(max(Crit(i).data)) + 100*sum(in(i,:).^2);   % coeffient 100 should be considered to be proper
        else
            % Evaluate over power flow
%             out(i) = (max(max(transpose(Crit(i).data))))^2 + sum(in(i,:).^2);
                out(i) = (sum(max(Crit(i).data)))^2 + sum(in(i,:).^2);
        end
    end    
