...
for i = 1:g_s_period
    for j = 1:g_L_class
        if g_label(1,i) == j
            out_reshape(i,:) = pso_out_1(:,j)'; 
        end
    end
end
out_reshape = out_reshape'; % transpose
...
xlswrite(filename_w2,g_load_train,1,'B2');      
xlswrite(filename_w2,transpose(1:g_steps),1,'A2');      
xlswrite(filename_w2,adj_train_load(1).data,2,'B2');      
xlswrite(filename_w2,transpose(1:g_steps),2,'A2');    

filename_w3 = 'Adjusted test load.xlsx';
xlswrite(filename_w3, g_load_test, 1, 'B2');       
xlswrite(filename_w3, transpose(1:g_steps),1,'A2');      
xlswrite(filename_w3, valid_flow(1).data,2,'B2');        
