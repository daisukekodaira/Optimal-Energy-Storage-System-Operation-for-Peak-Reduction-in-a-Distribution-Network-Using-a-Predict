    ...
    for day = 1:g_days
        if day == 1
            for hour = 1: g_s_period % 1:24
                hourly_histgram(hour).data = load_train((hour-1)*30+1:hour*30,day); % hourly histgram (for 24 hours) is composed here. Because of 2min data, 30 is multiplied.
            end
        else
            for hour = 1: g_s_period % 1:24
                hourly_histgram(hour).data = [hourly_histgram(hour).data load_train((hour-1)*30+1:hour*30,day)];
            end
        end
    end
