%% Read Input File------------------------------------------------------------------------
% The load for optimization
% g_load_train = Generate_input_load;
f1 = 'load.csv';
g_load_traing = csvread(f1);

% The load for validation
f2 = 'test.csv';
g_load_test = csvread(f2);
% g_amb_temp = xlsread(f2,1);
g_flag_CI = 0; % Utilize if confidence interval or not true=1

%% Utilize confidence interval------------------------------------------------------------------
if g_flag_CI ==1
    % Get CI
    flag = 1; % 1: worst_case, 2: best_case
    [load_train] = CI(g_load_traing, flag);
    % [load_CI_test] = CI(g_load_test, flag);

    % Shfit the load based on CI
    g_load_train = data_reshape(load_train);
    g_load_test = data_reshape(g_load_test);
    % g_load_test = data_reshape(load_CI_test);
end
...
g_line_capacity = 6;   % Around 10 is appropriate for real data. Interval must be 0.5 such as 9.5, 10.0 or 10.5. I will make it flexible later.
