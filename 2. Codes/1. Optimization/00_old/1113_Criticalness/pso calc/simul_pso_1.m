...
for position = 1:g_num_ESS+3
    if pos_graph(position) == 1 
        ...
        if non_ope == 1
            g_L_critical_best = L_critical_orig;
        end
        y(end+1).data = repelem(L_critical_best,g_coef); % 2min: 1*24, 30 = 720   15min:1*24,4 = 96
        ...
        % 7. Original Criticalness (training data)
        y(end+1).data = repelem(L_critical_orig(position,:),g_coef); % 2min: 1*24, 30 = 720   15min:1*24,4 = 96
