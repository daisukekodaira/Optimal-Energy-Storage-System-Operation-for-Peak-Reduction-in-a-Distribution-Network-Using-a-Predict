function [out] = pso_objective(in)
    ...
    for i = 1:n_
        ...
        % Objective Function
        out(i) = mx_val(i) + 0.01*sum(abs(in(i,:))); % the coefficient is found by try-and-error
%         out(i) = exp(mx_val(i)) + 0.01*sum(abs(in(i,:))); % the coefficient is found by try-and-error
    end
