...
[g_pred_min, g_pred_max] = Confd_range(raw_train_load);
run_pso;
...
for position = 1:g_num_ESS+3    
    if pos_graph(position) == 1
        ...
        % 6. Original range  (training data)
        [orig_mn, orig_mx] = Confd_range(raw_train_load);
        y(end+1).data = repelem(orig_mx(position,:), g_coef); % 2min: 1*24, 30 = 720   15min:1*24,4 = 96
        y(end).name = 'Predicted range(max)';
        y(end).color = 'm';
        y(end).yaxis = 'left';
        y(end).linestyle = '--';
        y(end).descrp = [1 0];
        y(end+1).data = repelem(orig_mn(position,:), g_coef); % 2min: 1*24, 30 = 720   15min:1*24,4 = 96
        ...
        % 7. Adjusted range (training data + ess operation)
        [adj_mn, adj_mx] = Confd_range(adj_train_load);   % mx has 5*24, mn has 5*24    
        ...
        % Calculate peak
        [pred_peak, peak_hour(1)] = max(orig_mx(1,:));
        [adjusted_peak, peak_hour(2)] = max(adj_mx(1,:));  % peak among whole days
        % Table
        LastName = {char(strcat('Predicted load upper bounrady (', name(position), ')')); char(strcat('Adjusted load upper boundary (', name(position), ')'))};
...
% 1. Predicted load
% -----------------------------------------------------------------------------------
histogram_desc(g_flag_hist,raw_train_load(1).data, 'Predicted load'); % disply the histogram or not. True = 1, False =0
histogram_desc(g_flag_hist,adj_train_load(1).data, 'Adjusted load'); % disply the histogram or not. True = 1, False =0
...
xlswrite(filename_w1,Peak_at_Substation,2,'B2');        % Sheet1: Write T-node(1) original energy[wh] power consumption
