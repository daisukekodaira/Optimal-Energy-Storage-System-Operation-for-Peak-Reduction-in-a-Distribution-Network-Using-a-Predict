...
function [cost] = constraints(in)   
    ...
    for m = 1:g_num_ESS % the number of ESS
        last_SOC = g_currentSOC(m) + sum(re_in(m,:)); %  re_in "+": Chrage. ESS becomes load. SOC increases.
        if (last_SOC < g_currentSOC(m) - g_margin(m)) || (g_currentSOC(m) + g_margin(m) < last_SOC)
    ...
    for m = 1:g_num_ESS % the number of ESS
        ...
        for i = 1:g_s_period
            ...
            if SOC(i) > g_max_SOC(m)
                soc_err = (10^3) + (10^3)*abs(SOC(i) - g_max_SOC(m));  
            elseif SOC(i) < g_min_SOC(m)
                soc_err = (10^3) + (10^2)*abs(g_min_SOC(m) - SOC(i));         
            else
                soc_err = 0;
            end
            cost = cost +soc_err;
